...
class MidiApp:
    ...
    def _initialize_content(self):
        self.open_file_button = tk.Button(
            master=self._frame2, text="Open midi file", command=self._open_midi_file)
        ...
        self.trie_button = tk.Button(
            master=self._frame2, text="Read midi-file to trie", command=self.read_file_to_trie)
        ...
        )-1, orient=tk.HORIZONTAL, label="Valitse Markovin ketju aste, jota käytetään kappaleen generoimisessa")
        self.mc_order_song_slider.set(9)
        self.mc_order_song_slider.pack(pady=20)

        self.set_tempo_slider = tk.Scale(
            master=self._frame2, from_=1, to=250, orient=tk.HORIZONTAL, label="Aseta tempo")
        self.set_tempo_slider.set(120)
        self.set_tempo_slider.pack(pady=10)

        self.choose_song_lenght_slider = tk.Scale(
            master=self._frame2, from_=1, to=200, orient=tk.HORIZONTAL, label="Valitse kappaleen pituus (nuoteissa)")
        ...
        self.reset_trie_button = tk.Button(
            master=self._frame2, text="Reset trie", command=self.reset_trie)
